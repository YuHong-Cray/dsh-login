 ...
 * @param opts.allowSelfService - instance mode: whether the administrator has
 *   allowed this user to configure their own model providers (undefined/false
 *   = not allowed; see lib/model-policy.js).
 * @param opts.error - optional error banner.
 */
export function renderAccountPage({ mode, username, admin, port, running, instanceHost, allowSelfService, error }) {
  const errHtml = error ? `<div class="alert" role="alert">${esc(error)}</div>` : ''
  if (mode === 'instance') {
    const policy = allowSelfService === true
      ? '<span class="badge ok">已允许自行设置</span>'
      : '<span class="badge down">已被管理员关闭</span>'
    // When the administrator closed self-service the endpoint is closed too,
    // so the account page does not offer a link that would only 403.
    const modelRow = allowSelfService === true
      ? `<div class="row"><label>模型管理</label><a class="btn" href="/dsh-login/models">我的模型（添加 / 删除 provider 与 API key）</a> ${policy}</div>`
      : `<div class="row"><label>模型管理</label><span style="color:var(--muted)">模型设置功能已关闭；如需添加 / 更改 provider 或 API key，请联系管理员</span> ${policy}</div>`
    const body = `
${errHtml}
 ...
 * @param opts.users - [{username, admin, status, port, running, createdAt,
 *   lastLoginAt, modelSelfService}] (`modelSelfService` = whether the user may
 *   configure their own model providers in their instance).
...
export function renderUsersPage({ users, self, instanceHost, error }) {
  ...
  const rows = users.map((u) => {
    ...
    const allowed = u.modelSelfService === true
    // The switch is what 「批准」 means for model parameters: denied (the
    // default) leaves the user's 「我的模型」 page read-only; allowed lets them
    // add/change/remove providers themselves without a per-change approval.
    const modelPolicyCell = u.admin
      ? '<span style="color:var(--faint)">—</span>'
      : allowed ? '<span class="badge ok">已允许</span>' : '<span class="badge">已禁止</span>'
    ...
    const policyBtn = u.admin
      ? ''
      : allowed
        ? `<button class="btn danger" data-policy="deny" data-user="${esc(u.username)}">禁止自行设置模型</button>`
        : `<button class="btn primary" data-policy="allow" data-user="${esc(u.username)}">允许自行设置模型</button>`
...
<td>${statusCell}</td>
<td>${modelPolicyCell}</td>
<td class="mono">${portCell}</td>
<td>${runCell}</td>
<td class="mono">${esc(u.createdAt ?? '—')}</td>
<td class="mono">${esc(u.lastLoginAt ?? '从未')}</td>
<td class="actions">${approveBtn}${rejectBtn}${policyBtn}${enterBtn}${openBtn}${modelBtn}${delBtn}</td>
</tr>`
  }).join('\n')
  const body = `
${errHtml}${pendingHint}
<table>
<thead><tr><th>账号</th><th>角色</th><th>状态</th><th>自行设置模型</th><th>实例端口</th><th>实例状态</th><th>创建时间</th><th>最近登录</th><th>操作</th></tr></thead>
<tbody>
${rows}
</tbody>
</table>
...
document.querySelectorAll('button[data-policy]').forEach(function (btn) {
  btn.addEventListener('click', async function () {
    var u = btn.getAttribute('data-user')
    var allow = btn.getAttribute('data-policy') === 'allow'
    var word = allow ? '允许' : '禁止'
    if (!allow && !window.confirm('确认禁止账号 "' + u + '" 自行设置模型参数？\\n该用户的「我的模型」页将变为只读：添加 / 更改 / 删除 provider 与 API key 都需管理员操作。')) return
    btn.disabled = true
    try {
      var res = await fetch('/dsh-login/users/model-policy', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ user: u, allow: allow }),
      })
      var data = await res.json().catch(function () { return {} })
      if (!res.ok) { window.alert(word + '失败：' + (data.error || res.status)); btn.disabled = false; return }
      location.reload()
...
`
  const note = '「通过 / 拒绝」是账号审批：新注册的账号默认「待审批」，审批通过后才能登录（管理员账号不受审批约束）。'
    + '「自行设置模型」是该用户能否在自己的「我的模型」里添加 / 更改 provider 与 API key：默认禁止，点「允许自行设置模型」即为批准（此后无需逐次审批）；'
    + '禁止后其页面变为只读，模型仍可由管理员在「模型」页配置。'
 ...
 *
 * NOTE: this function renders the OPEN page. It is only reached when the
 * administrator's per-user switch allows self-service; otherwise the routes
 * answer with {@link renderModelsClosedPage} and never call this renderer.
...
/**
 * The page shown when the administrator has CLOSED self-service for this user.
 *
 * The plugin serves this (HTTP 403) for every `/dsh-login/models*` request
 * while the switch is off: the endpoint is closed, not merely read-only — no
 * provider list, no form, no JSON actions. Model configuration for the user is
 * then the administrator's job (user-management → 模型), and the user can still
 * pick whatever the administrator granted in the normal GUI model selector.
 * @param opts.port - the instance port (display only).
 */
export function renderModelsClosedPage({ port }) {
  const body = `
<div class="alert warn" role="alert">管理员已关闭「模型设置」功能：你没有权限在本环境添加 / 更改 provider 或 API key，此页面及其接口均不可用。</div>
<div class="row"><label>当前环境</label><span>独立实例</span>${port > 0 ? ` <span class="badge ok">端口 ${String(port)}</span>` : '<span class="badge">端口未知</span>'}</div>
<div class="row"><label>如何修改模型</label><span style="color:var(--muted)">请联系管理员：管理员可在「用户管理」中为你开启「允许自行设置模型」，或直接用「用户管理 → 模型」为你配置。</span></div>
<div class="row"><label></label><a class="btn" href="/dsh-login/account">返回账户中心</a></div>
`
  const note = '已由管理员授予的模型仍可正常使用（会话里选择即可）；本开关只决定你是否能改动模型参数。'
  return shell('模型设置已关闭', 'dsh-login · 该功能由管理员逐人开启', body, note)
}
