...
export class LoginDatabase {
   ...
   * @returns the row ({username, password_hash}) or undefined when absent.
   */
  async findByUsername(username) {
    const t = this.table()
    try {
      const [rows] = await this.poolInstance().query(
        `SELECT username, password_hash, status FROM ${t} WHERE username = ? LIMIT 1`,
        [username],
      )
      return rows?.[0]
    } catch (err) {
   ...
   * @returns [{username, created_at, last_login_at, status}] ordered by username.
   */
  async listUsers() {
    const t = this.table()
    try {
      const [rows] = await this.poolInstance().query(
        `SELECT username, created_at, last_login_at, status FROM ${t} ORDER BY username`,
      )
      return rows
    } catch (err) {
      this.lastError = err.message
